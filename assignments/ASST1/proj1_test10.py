...
def nnObjFunction(params, *args):
    ...
    # computing equation (12) : 
    term1 = -1.0 * ( -1.0 - z_hidden)
    #term2 = np.transpose(z_hidden)
    term3 = np.multiply(term1,z_hidden)
    #print term3.shape
    term4 = np.dot(lambda_l_mat,w2[:,:-1])
    #print term4.shape
    term5 = np.multiply(term3,term4)

    term6 = np.transpose(term5)

    term7 = np.dot(term6,train_data_bias)
    #print np.transpose(term7)
    ...
    pickleFile = open("params.pickle1", 'wb')
